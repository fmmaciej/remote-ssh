...
remote_ssh_cmd_git_status_collect_agent() {
  REMOTE_SSH_GIT_STATUS_SSH_AUTH_SOCK="${SSH_AUTH_SOCK:-}"
  REMOTE_SSH_GIT_STATUS_SSH_ADD_FOUND=0
  REMOTE_SSH_GIT_STATUS_SSH_ADD_OUTPUT=""
  REMOTE_SSH_GIT_STATUS_SSH_ADD_EXIT=127

  command -v ssh-add >/dev/null 2>&1 || return 0
  REMOTE_SSH_GIT_STATUS_SSH_ADD_FOUND=1

  set +e
  REMOTE_SSH_GIT_STATUS_SSH_ADD_OUTPUT="$(ssh-add -l 2>&1)"
  REMOTE_SSH_GIT_STATUS_SSH_ADD_EXIT=$?
  set -e
}
